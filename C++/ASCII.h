#ifndef SYSTEMS_H_INCLUDED
#define SYSTEMS_H_INCLUDED

//INITIAL START UP FUNCTIONS
void Welcome(void)
{
    printf("                     _                                \n");
    printf("                   (` '.                              \n");
    printf("        _.--.     /   a '-.                           \n");
    printf("        '->  \\ __/     __.'   ______ ______  _____   \n");
    printf("         <_   |.'  /`-._)`-   | ___ \\| ___ \\|  __ \\  \n");
    printf("           >--'    \\          | |_/ /| |_/ /| |  \\/  \n");
    printf("        .--;'       |--.      |    / |  __/ | | __   \n");
    printf("  ,-    _\\.'        |   \\_    | |\\ \\ | |    | |_\\ \\  \n");
    printf(" //    /-'    \\    / `\\__/    \\_| \\_|\\_|     \\____/  \n");
    printf("| |  .'       /_._ |                                  \n");
    printf("\\  '-'  _/  --.  \\ ' -.                             \n");
    printf("'.___.-' \\____/jgs\\___/                             \n\n");

    printf("WELCOME TO MY RPG\n\n");
}

void pageBreak(void)
{
    printf("                                                           \n");
    printf("               ,                                           \n");
    printf("              (@|                                          \n");
    printf(" ,,           ,)|_____________________________________     \n");
    printf("//\\\\8@8@8@8@8@8 / _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ \\ \n");
    printf("\\\\//8@8@8@8@8@8 \\_____________________________________/ \n");
    printf(" ``           `)|                                          \n");
    printf(" jgs          (@|                                          \n");
    printf("              `                                            \n");
    printf("                                                           \n");

}

void Victory(void)
{
    printf("\n");
    printf(" _   _  _____  _____  _____  _____ ______ __   __\n");
    printf("| | | ||_   _|/  __ \\|_   _||  _  || ___ \\\\ \\ / /\n");
    printf("| | | |  | |  | /  \\/  | |  | | | || |_/ / \\ V / \n");
    printf("| | | |  | |  | |      | |  | | | ||    /   \\ /  \n");
    printf("\\ \\_/ / _| |_ | \\__/\\  | |  \\ \\_/ /| |\\ \\   | |  \n");
    printf(" \\___/  \\___/  \\____/  \\_/   \\___/ \\_| \\_|  \\_/  \n");
    printf("\n");
}

void Defeat(void)
{
    printf("\n");
    printf("______ ___________ _____  ___ _____ \n");
    printf("|  _  \\  ___|  ___|  ___|/ _ \\_   _|\n");
    printf("| | | | |__ | |_  | |__ / /_\\ \\| |  \n");
    printf("| | | |  __||  _| |  __||  _  || |  \n");
    printf("| |/ /| |___| |   | |___| | | || |  \n");
    printf("|___/ \\____/\\_|   \\____/\\_| |_/\\_/  \n");
    printf("\n");
}

void Egg1(void)
{
    printf("           .-\"-.    \n");
    printf("         .'     '.  \n");
    printf("        /         \\ \n");
    printf("       :           ;\n");
    printf("       |           |\n");
    printf("       :           :\n");
    printf("        \\         / \n");
    printf("         `.     .'  \n");
    printf("     jgs   `~~~`    \n");
}

void Egg2(void)
{
    printf("           .-\"-.    \n");
    printf("         .'     '.  \n");
    printf("        /         \\ \n");
    printf("       :           ;\n");
    printf("       |           |\n");
    printf("       :      'Y   :\n");
    printf("        \\       \\ / \n");
    printf("         `.     .'  \n");
    printf("     jgs   `~~~`    \n");
}

void Egg3(void)
{
    printf("           .-\"-.    \n");
    printf("         .'     '.   \n");
    printf("        /         \\ \n");
    printf("       :        \\/ ; \n");
    printf("       |    __  /  | \n");
    printf("       :      'Y   : \n");
    printf("        \\       \\ / \n");
    printf("         `.     .'   \n");
    printf("     jgs   `~~~`     \n");
}

void Egg4(void)
{
    printf("           .-\"-.    \n");
    printf("         .'     '.   \n");
    printf("        /    _\\_  \\ \n");
    printf("       :        \\/ ; \n");
    printf("       |   \\__  /  | \n");
    printf("       :   /  'Y   : \n");
    printf("        \\       \\ / \n");
    printf("         `.     .'   \n");
    printf("     jgs   `~~~`     \n");
}

void Egg5(void)
{
    printf("           .-\"-.    \n");
    printf("         .'  \\/ '.   \n");
    printf("        /\\/  _\\_  \\ \n");
    printf("       : _\\     \\/ ; \n");
    printf("       |   \\__  /  | \n");
    printf("       :  _/  `Y-- : \n");
    printf("        \\ /  .--\\ / \n");
    printf("         `. /   .'   \n");
    printf("     jgs   `~~~`     \n");
}

void Egg6(void)
{
    printf("           .-\\\"-.         \n");
    printf("         .' `\\/_'.         \n");
    printf("        /\\/ \\_\\_  \\     \n");
    printf("       : _\\ /   \\//       \n");
    printf("       |/  \\__/ / \\       \n");
    printf("       : \\_/  `Y--':       \n");
    printf("        \\ / \\.--\\ /      \n");
    printf("         `. /   .'  /\\     \n");
    printf("     jgs   `~~~` .-.`\"     \n");
    printf("                 `'`        \n");
}

void Egg7(void)
{
    printf("           .-\"-.         \n");
    printf("         .' `\\/_'.         \n");
    printf("        /\\/{\\_\\/\\/\\     \n");
    printf("        `\\ | \" | /\\       \n");
    printf("       //:\\(o o)//\\\\       \n");
    printf("       |/  \\_^/\\_/\\|       \n");
    printf("       : \\_/  `Y--':      \n");
    printf("   /^|  \\ / \\.--\\ /    |\\     \n");
    printf("   `\" .-.`. /   .'  /\\ `~     \n");
    printf("     jgs`  `~~~` .-.`\"        \n");
    printf("                 `'`        \n");
}

void Egg8(void)
{
    printf("      _(,  {\\V/}  ,)_         \n");
    printf("        /\\ |\" | /\\         \n");
    printf("       //\\\\(o o)//\\\\ \\/\\-'^^\\/\\/     \n");
    printf("       ||::/ ^ \\::||  '.     .'       \n");
    printf("       : \\_/\\_/`Y-':    '-.-'       \n");
    printf("   /^|  \\ / \\.--\\ /    |\\       \n");
    printf("   `\" .-.`. /   .'  /\\ `~      \n");
    printf("     jgs`  `~~~` .-.`\"     \n");
    printf("         /'.     `'`     \n");
    printf("         ``        \n");
}

void Egg9(void)
{
    printf("      _(,  {\\V/}  ,)_         \n");
    printf("        /\\ |\" | /\\         \n");
    printf("       //\\\\(o o)//\\\\ \\/\\-'^^\\/\\/     \n");
    printf("       ||::/ ^ \\::||  '.     .'    \n");
    printf("       : \\_/\\_/`Y-':    '-.-'     \n");
    printf(" /^|\\ /|'  \" \"/  '|-\\ /|\\          \n");
    printf(" `\" .`.     )I(    .'/\\ `~          \n");
    printf("   jgs``~  \"\"`\"\"  `-.`\"             \n");
    printf("       /'.          `'`             \n");
    printf("       ``                           \n");
}

void Egg10(void)
{
    printf("      _(,  {\\V/}  ,)_    \n");
    printf("        /\\ | \" | /\\      \n");
    printf("       //\\\\(o o)//\\\\     \n");
    printf("       ||::/ ^ \\::||     \n");
    printf("       |||((   ))|||     \n");
    printf("       |'  \\\" \"/  '|     \n");
    printf("            )I(          \n");
    printf("   jgs     \"\"`\"\"         \n");
}

void DragonHatch()
{
    int i;
    Egg1();
    for(i=0;i<200000000;i++);
    Egg2();
    for(i=0;i<200000000;i++);
    Egg3();
    for(i=0;i<200000000;i++);
    Egg4();
    for(i=0;i<200000000;i++);
    Egg5();
    for(i=0;i<200000000;i++);
    Egg6();
    for(i=0;i<200000000;i++);
    Egg7();
    for(i=0;i<200000000;i++);
    Egg8();
    for(i=0;i<200000000;i++);
    Egg9();
    for(i=0;i<200000000;i++);
    Egg10();
}

/*eventualy quest will return a int for which quest is choosen
You could use the pointer int requirement here later*/

void getQuest(void)
{
    printf("\nBrave Adventurer! Welcome to our most humble town!");
    printf("\nMy name is Isaac Alexi Carroll! I am the mayor of this here town.");
    printf("\nCreatures of all manner have taken camp on the outskirts.");
    printf("\nA Terrible Dragon has recently arrived and has become their leader\n");
    printf("\nKill The Monsters! Kill The Dragon!\n");
    printf("I think if you were to kill 5 Monsters, the Dragon would be so enraged that he will fight you himself!\n");
    printf("Folks are terrified to leave their homes, much less fight of monsters.\n");
    printf("Please Help Us!");
}

/*Pretty Picture
    printf("   ,   A           {}     \n");
    printf("  / \\, | ,        .--.    \n");
    printf(" |    =|= >      /.--.\\   \n");
    printf("  \\ /` | `       |====|   \n");
    printf("   `   |         |`::`|   \n");
    printf("       |     .-;`\\..../`;_.-^-._     \n");
    printf("      /\\\\/  /  |...::..|`   :   `|   \n");
    printf("      |:'\\ |   /'''::''|   .:.   |   \n");
    printf("       \\ /\\;-,/\\   ::  |..:::::..|   \n");
    printf("       |\\ <` >  >._::_.| ':::::' |   \n");
    printf("       | `\"\"`  /   ^^  |   ':'   |   \n");
    printf("       |       |       \\    :    /   \n");
    printf("       |       |        \\   :   /    \n");
    printf("       |       |___/\\___|`-.:.-`     \n");
    printf("       |        \\_ || _/             \n");
    printf("       |        <_ >< _>             \n");
    printf("       |        |  ||  |             \n");
    printf("       |        |  ||  |             \n");
    printf("       |       _\\.:||:./_            \n");
    printf("       | jgs  /____/\\____\\           \n");
    printf("                                     \n");
*/


#endif // SYSTEMS_H_INCLUDED
